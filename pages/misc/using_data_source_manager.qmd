...
4.  You must now select only the 6 buurten that obey to these criteria:
    ...
    -   They belong to the municipalities of Rotterdam and Schiedam (`gemeentenaam` = "Rotterdam" OR `gemeentenaam` = "Schiedam") **AND**
    ...
    **To select these neighbourhoods:**
    ...
    ```         
    "gemeentenaam" IN ('Schiedam', 'Rotterdam') AND "buurtnaam" IN ('Stationsbuurt', 'Singelkwartier', 'Natuurkundigenbuurt', 'Marconibuurt', 'Glasbuurt', 'Nieuw-Mathenesse', 'Wetenschappersbuurt', 'Newtonbuurt', 'Boylebuurt', 'Oud Mathenesse', 'Nieuw Mathenesse', 'Witte Dorp', 'Spangen', 'Tussendijken', 'Bospolder', 'Schiemond', 'Nieuwe Westen', 'Delfshaven', 'Middelland', 'Dijkzigt')
    ```
    ...
    -   Before saving, we need to add the area calculation and drop unwanted columns:
        -   Click on the fields list and uncheck all columns except:
            ...
            -   `gemeentenaam` (name of the municipality)
                ...
                ::: callout-tip
                You can use the `deselect all` button and then manually select only the 3 fields you want to keep.
                :::
    -   Click `OK` to save
    ...
    **Add the area column:**
    ...
    ![Figure 2 - Calculating the area in the expression box](using_data_source_manager/fig%202.png)
...
5.  Dissolve now the geometries of the 6 buurten:
    ...
    -   Click `OK`
        ...
        When unchecking all atributes, it will give you back an empty attribute table.
        :::

    ![Figure 3 - Dissolved neighbourhood boundary](using_data_source_manager/fig%203.png)
...
6.  Compute the bounding box/extents of `buurten_dissolved`:
    ...
    ![Figure 4 - Bounding box of the study area](using_data_source_manager/fig%204.png)
...
7.  Finally, compute a buffer of 200 m around the bbox layer:
    ...
    ![Figure 5 - 200m buffer around bounding box](using_data_source_manager/fig%205.png)
...
11. Load a WFS layer:
    ...
    -   Find and select `klimaat:PANDEN_WATEROVERLAST_HITTE_GROEN` (Basiskaarten klimaatadaptatie panden - wateroverlast, hitte en biodiversiteit 2022)
    -   Click `Add` at the bottom
    -   Click `Close`

    ![](using_data_source_manager/fig%206.png)
...
12. Clip the layer to your study area:
    ...
    -   You can now remove the original (unclipped) WFS layer to improve performance. Make sure to save the temporary layers you want to keep.
...
13. Explore the layer:
    ...
    -   Explore the climate adaptation attributes like water overlay risk, heat stress, and biodiversity scores
        ...
        ![](using_data_source_manager/fig 7.png)
